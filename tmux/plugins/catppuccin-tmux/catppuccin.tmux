...
build_window_format() {
  ...
  if [ "$fill" = "none" ]
  then
    local show_left_separator="#[fg=color1,bg=color1,nobold,nounderscore,noitalics]$window_left_separator"
    local show_number="#[fg=color1,bg=color1]$number"
    local show_middle_separator="#[fg=color1,bg=color1,nobold,nounderscore,noitalics]$window_middle_separator"
    local show_text="#[fg=color1,bg=color1]$text"
    local show_right_separator="#[fg=color1,bg=color1]$window_right_separator"
  ...
  if [ "$fill" = "number" ]
  then
	local show_number="#[fg=foreground,bg=color1]$number" # NOTE: color1 = color behind the number of the window, foreground = number (text) color
    local show_middle_separator="#[fg=foreground,bg=color0,nobold,nounderscore,noitalics]$window_middle_separator" # NOTE:
    local show_text="#[fg=foreground,bg=color0]$text" # NOTE:
    ...
    then
      ...
      local show_left_separator="#[fg=color5,bg=color1,nobold,nounderscore,noitalics]$window_left_separator"
      local show_right_separator="#[fg=color5,bg=color1]$window_right_separator"
    fi
    ...
    then
      ...
      local show_left_separator="#[fg=color1,bg=background]$window_left_separator"
    fi
...
build_status_module() {
  ...
  then
    ...
    local show_left_separator="#[fg=foreground,bg=background,nobold,nounderscore,noitalics]$status_left_separator"

	# NOTE: second color = background of the round < containing an icon
    local show_icon="#[fg=color4,bg=color15,nobold,nounderscore,noitalics]$icon "

	# NOTE: First color = background color of the |==| containing {Pane number, tmux, hour}
	local show_text="#[fg=foreground,bg=background] $text"

    local show_right_separator="#[fg=color4,bg=color15,nobold,nounderscore,noitalics]$status_right_separator"

    if [ "$status_connect_separator" = "yes" ]
    then
      local show_left_separator="#[fg=foreground,bg=background,nobold,nounderscore,noitalics]$status_left_separator"
      local show_right_separator="#[fg=foreground,bg=background,nobold,nounderscore,noitalics]$status_right_separator"
  ...
  then
    ...
    local show_text="#[fg=$thm_bg,bg=background]$text"

    local show_right_separator="#[fg=foreground,bg=background,nobold,nounderscore,noitalics]$status_right_separator"

    if [ "$status_connect_separator" = "yes" ]
    then
      local show_left_separator="#[fg=foreground,nobold,nounderscore,noitalics]$status_left_separator"
      local show_right_separator="#[fg=foreground,bg=background,nobold,nounderscore,noitalics]$status_right_separator"
  ...
  if [ $(($index)) -eq 0  ]
  then
	  # NOTE: first color = background of the little < on the left of the {window number, "tmux", hour}
	  # NOTE: second color = background color on the left side
      local show_left_separator="#[fg=color1,bg=$thm_bg,nobold,nounderscore,noitalics]$status_left_separator"
  fi
...
main() {
  ...
  # NOTE: background color of the whole horizontal bar
  set status-bg "${background}"
  set status-justify "left"
  set status-left-length "100"
  set status-right-length "100"

  # messages
  set message-style "fg=${color1},bg=${color1},align=centre"
  set message-command-style "fg=${color1},bg=${color1},align=centre"

  # panes
  # set pane-border-style "fg=${thm_gray}"
  set pane-border-style "fg=${color0}"
  # set pane-active-border-style "fg=${thm_blue}"
  set pane-active-border-style "fg=${color1}"

  # windows
  # setw window-status-activity-style "fg=${thm_fg},bg=${thm_bg},none"
  setw window-status-activity-style "fg=${foreground},bg=${background},none"
  setw window-status-separator ""
  # setw window-status-style "fg=${thm_fg},bg=${thm_bg},none"
  setw window-status-style "fg=${color1},bg=${color1},none"
  ...
  setw clock-mode-colour "${color4}"
  # setw mode-style "fg=${thm_pink} bg=${thm_black4} bold"
  setw mode-style "fg=${color1} bg=${color4} bold"
